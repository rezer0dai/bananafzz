...
impl Drop for FuzzyState {
    fn drop(&mut self) {
        if let Err(e) = bananaq::dtor(&self.banana) {
            println!("[fuzzing] delayloaded drop of {} for bananaq#{:X} <{e}>", self.istate.state().info().name, self.qid);
        }
    }
}
impl FuzzyState {
...
/// - check for end-conditions of fuzz and quit
    pub fn fuzz(istate: Box<dyn IFuzzyObj>) -> Result<thread::JoinHandle<Result<(), String>>, &'static str> {
        Ok(thread::spawn(move || {
            let banana = istate.state().info().bananaq();

            if !bananaq::push(&banana, &istate)? {
                return Err(format!("[bananaq] failed to push {}", 
                        istate.state().info().name))
            }
            let racer = 0 != istate.state().level();

            let mut fuzzy_state = FuzzyState::new(banana
                .upgrade()
                .ok_or(format!("[bananaq] bananaq#? is no longer"))?,
                istate);

            if let Err(e) = fuzzy_state.init() {
                return Err(format!("[bananaq] FuzzyState {} failed to init with message <{}>", 
                        fuzzy_state.istate.state().info().name, e))
            }
            if !bananaq::ctor_notify(fuzzy_state.istate.state().info()) {
                return Err(format!("[bananaq] FuzzyState {} <uid:{:?}> failed to register to queue", 
                    fuzzy_state.istate.state().info().name, fuzzy_state.istate.state().info().uid()))
            }
