...
impl Arg {
    ...
    pub fn data(&self) -> &[u8] { self.data.data() }
    pub fn data_mut(&mut self) -> &mut[u8] { self.data.data_mut() }
    ...
    pub fn primitive_arg(generator: Box<dyn IArgLeaf>) -> Arg {
        ...
        if generator.size() > mem::size_of::<usize>() {
            panic!("trying to mess up with primitive arg, too huge size for {} : {}", generator.name(), generator.size());
        }
        Arg::new(
            "primitive_",
            generator,
    ...
    pub fn data_mut_unsafe<T>(&mut self) -> &mut T {
        if mem::size_of::<T>() > self.data.len() {
            panic!("trying to load from argument '{}' complex struct of size {} vs {}", self.name, mem::size_of::<T>(), self.data.len());
        }
    ...
    pub fn data_const_unsafe<T>(&self) -> &T {
        if mem::size_of::<T>() > self.data.len() {
            panic!("trying to read from argument '{}' complex struct of size {} vs {}", self.name, mem::size_of::<T>(), self.data.len());
        }
        assert!(mem::size_of::<T>() <= self.data.len());
